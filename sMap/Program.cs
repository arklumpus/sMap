...
Restart: the watchdog forces the MCMC sampler to assume that the analysis has converged, then discards everything and restarts the MCMC sampling from scratch (expecting that the deadlock will not happen again).", v => { MCMC.watchdogAction = Enum.Parse<MCMC.WatchdogActions>(v); } },
